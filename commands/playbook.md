...
If the argument is a topic (e.g., "frequency", "andromeda", "benchmarks", "TOF", "creative", "email"), search the playbook chunk files in `${CLAUDE_PLUGIN_ROOT}/skills/playbook-reference/references/` for that topic. Read the most relevant chunk(s) and present a concise summary of what the playbook says about that topic.
