---
name: ga4-audit-v2
description: "Run a deep GA4 analytics audit — traffic acquisition, conversion funnels, cross-platform attribution reconciliation, event tracking validation — then write a standardized evidence JSON file. Triggers on: 'audit their GA4', 'analytics audit', 'audit Google Analytics', 'GA4 tracking audit', 'attribution audit'."
---
...
**Standard setup:** Follow `audit-shared/reference/audit-lifecycle.md` → "Before You Start" section. This covers manifest checks, evidence directory setup, and AOV-conditional playbook loads.

**GA4-specific loads:**
- `reference/nav-ga4.md` — Read BEFORE opening GA4. Contains exact navigation paths, date range workarounds, report customization, and GA4 Gotchas (Traps 1-10). **Do not navigate GA4 without reading this first.**
- `references/benchmarks.md` — Load at setup (standard per audit-lifecycle)
- `references/measurement.md` — **Deferred: load at Phase 7** (cross-platform attribution). Not needed for Phases 1-6.
...
> **Load now:** `references/measurement.md` — contains source-of-truth stack, reconciliation methodology, expected gap ranges.
...
**Location:** Per audit-lifecycle.md (Disruptive or {client_name} evidence directory).

**Schema:** Follow `audit-shared/reference/evidence-schema-quick.md`. Key GA4-specific mappings:
...
**Evidence labeling:** Follow `audit-shared/reference/evidence-rules.md` (5-label system: OBSERVED, CALCULATED, INFERENCE, ASSUMPTION, DATA_NOT_AVAILABLE).

---

## After the Audit

Follow `audit-shared/reference/audit-lifecycle.md` → "After the Audit" section (save JSON, update manifest, flag critical issues, save working notes).
