...
**Data sources:**
- Revenue: Shopify evidence → `account_overview` (OBSERVED)
- COGS: Shopify evidence → product cost data, OR client-provided (OBSERVED if available)
...
**Numerator:** Shopify total revenue from Shopify evidence → `account_overview` total revenue. If no Shopify evidence, use the best available proxy (GA4 revenue, or sum of platform revenue as last resort — label ASSUMPTION).
...
```
Processing Cost per Order = AOV × 0.029 + $0.30 (Shopify Payments standard)
```
...
```
Shopify evidence exists?
├── YES → Use Shopify revenue as source of truth
│   ├── COGS available?
...
1. **Source of truth statement:** "Financial data sourced from {Shopify/Platform estimates}. Label: {OBSERVED/ASSUMPTION}."
