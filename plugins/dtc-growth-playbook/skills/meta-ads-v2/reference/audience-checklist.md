...
Reference for the meta-ads-v2 audit skill. Systematic checklist for evaluating Meta audience targeting strategy, funnel segmentation, overlap detection, exclusion hygiene, and Advantage+ audience behavior. Draws from `protocols/playbook/andromeda.md` (targeting hierarchy, Advantage+ behavior) and `protocols/playbook/scaling-frequency.md` (audience saturation signals).
