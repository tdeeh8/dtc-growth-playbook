---
name: shopify-audit-v2
description: "Run a Shopify store profitability audit — revenue metrics, AOV analysis, customer cohorts, LTV calculations, refund rates, discount impact — then write a standardized evidence JSON file. This is the financial source of truth. Triggers on: 'audit their Shopify', 'profitability audit', 'store data audit', 'Shopify revenue audit'."
---
...
Follow **`audit-shared/reference/audit-lifecycle.md`** → Standard Setup (manifest, evidence directory, AOV-conditional playbook chunks).

**Shopify-specific additions:**
- Also load `references/measurement.md` — "Shopify is the financial source of truth" section, MER calculation, source-of-truth stack
- If no manifest exists, also ask: whether COGS data is entered in Shopify
- Do NOT load `reference/nav-shopify.md` yet — load it at Phase 1 start (phase-gated)
...
**Now load** `reference/nav-shopify.md` for Shopify admin UI patterns and date picker gotchas.
...
Follow schema per **`audit-shared/reference/evidence-schema-quick.md`**. Apply labeling rules per **`audit-shared/reference/evidence-rules.md`**.
...
Follow **`audit-shared/reference/audit-lifecycle.md`** → Standard Closeout (save evidence JSON, update manifest, flag criticals, save working notes).
