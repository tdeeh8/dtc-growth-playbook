---
name: klaviyo-audit-v2
description: "Run a deep Klaviyo email/SMS audit — flow performance, campaign metrics, list health, segmentation quality, revenue attribution — then write a standardized evidence JSON file. Triggers on: 'audit their Klaviyo', 'email audit', 'audit email marketing', 'flow performance audit', 'email revenue audit'."
---
...
If MCP tools NOT connected, use `reference/nav-klaviyo.md` for browser navigation patterns.
...
Follow `audit-shared/reference/audit-lifecycle.md` for standard setup (playbook loading, manifest check, evidence directory), then:

**Klaviyo-specific reference files (load now):**
- `reference/nav-klaviyo.md` — MCP + browser navigation
...
**Browser path:**
Navigate dashboard → record summary metrics → Flows tab → Campaigns tab → Audience > Lists & Segments. See `reference/nav-klaviyo.md`.
...
**Before this phase, read:** `reference/flow-checklist.md`, `references/email-sms.md`, `references/post-purchase.md` (if not already loaded).
...
**Before this phase, read:** `references/list-building.md` (if not already loaded).
...
Conform to `audit-shared/reference/evidence-schema-quick.md`.
...
Read `audit-shared/reference/evidence-rules.md` for the full 5-label system (OBSERVED, CALCULATED, INFERENCE, ASSUMPTION, DATA_NOT_AVAILABLE).
...
Follow `audit-shared/reference/audit-lifecycle.md` > After the Audit section. Platform = "klaviyo".

---

## Working Notes

Follow template in `audit-shared/reference/audit-lifecycle.md` > Working Notes section.
...
Load from `references/benchmarks.md` > Email/Klaviyo section.
...
Read `audit-shared/reference/evidence-rules.md` > Anti-Hallucination Rules section.
