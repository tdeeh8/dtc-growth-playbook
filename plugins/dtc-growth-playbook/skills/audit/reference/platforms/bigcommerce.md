...
1. `reference/audit-lifecycle.md` — setup, manifest check, playbook loading, closeout, working notes template
2. `reference/evidence-rules.md` — 5-label system (OBSERVED/CALCULATED/INFERENCE/ASSUMPTION/DATA_NOT_AVAILABLE), anti-hallucination rules, data quality standards
3. `reference/evidence-schema-quick.md` — JSON schema structure, raw_metrics keys, priority/severity enums
...
- `reference/playbook/measurement.md` — "Ecommerce platform is financial source of truth" section, MER calculation, source-of-truth stack

**Navigation reference:** Read `reference/platform-refs/nav-bigcommerce.md` for Control Panel UI patterns — Analytics section, Orders filtering, date pickers, report generation, export workflows, API fallbacks, known gotchas.
