...
Reference for the meta-ads-audit-v2 audit skill. Systematic checklist for evaluating Meta audience targeting strategy, funnel segmentation, overlap detection, exclusion hygiene, and Advantage+ audience behavior. Draws from `reference/playbook/andromeda.md` (targeting hierarchy, Advantage+ behavior) and `reference/playbook/scaling-frequency.md` (audience saturation signals).
