...
## COGS Estimation (When Shopify COGS = $0 or Client Won't Share)

**If Shopify returns COGS = $0 or null:** This is common — most Shopify stores don't populate product cost fields. Do NOT treat $0 COGS as real data (i.e., don't report 100% gross margin). Instead:

1. Check if vertical was captured during audit setup. If yes, use the estimate table below.
2. If vertical is unknown, ask the user: "Shopify doesn't have COGS populated. What's the approximate product margin for this business? Or tell me the vertical and I'll estimate." Do this BEFORE running profitability analysis, not after.
3. Always run sensitivity analysis at two COGS points (low and high end of the vertical range).
