...
**When it runs:** If Google Ads passed the data-source health check in Step 1.3.5 AND is included in the platforms being audited — regardless of triage score (GREEN, YELLOW, or RED).
...
3. Read `reference/synthesizer.md` AND `reference/docx-template.md` (if present); invoke the built-in `docx` skill via the Skill tool
...
| `reference/docx-template.md` | At report generation — DOCX structure/styling, includes `chartImage()` helper for embedding chart PNGs |
