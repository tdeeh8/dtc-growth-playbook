...
> Start or check progress on a client audit. Creates the audit manifest, sets up the evidence directory, and tells you which platform audits to run in order.

## Usage

```
/audit-start                    → Prompts for client name and details
/audit-start Acme Co     → Starts (or checks status of) audit for Acme Co
```
...
2. Creates the evidence directory at the correct path (per your file routing rules)
...
| File | Purpose |
|------|---------|
| `SKILL.md` | Full orchestrator logic |
| `reference/evidence-schema.json` | JSON Schema contract for evidence files |
| `reference/manifest-format.md` | Manifest template and field definitions |
